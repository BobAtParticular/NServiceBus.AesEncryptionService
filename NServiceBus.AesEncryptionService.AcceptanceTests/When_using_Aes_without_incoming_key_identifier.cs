namespace NServiceBus.AcceptanceTests.Encryption
{
    ...
    public class When_using_Aes_without_incoming_key_identifier : NServiceBusAcceptanceTest
    {
        ...
        public class Sender : EndpointConfigurationBuilder
        {
            public Sender()
            {
                EndpointSetup<DefaultServer>(builder => builder.AesEncryptionService("will-be-removed-by-transport-mutator", Encoding.ASCII.GetBytes("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")))
        ...
        public class Receiver : EndpointConfigurationBuilder
        {
            public Receiver()
            {
                ...
                EndpointSetup<DefaultServer>(builder => builder.AesEncryptionService("new", keys, expiredKeys));
            }
        ...
        class RemoveKeyIdentifierHeaderMutator : IMutateIncomingTransportMessages, INeedInitialization
        {
            ...
            {
                context.Headers.Remove(EncryptionHeaders.KeyIdentifier);
                return Task.FromResult(0);
            }
